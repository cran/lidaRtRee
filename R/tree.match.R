...
#' @param deltaGround numeric. buffer around trunk position : absolute value
#' @param hPrec numeric. buffer around apex position : proportion of reference tree height
#' @param computeStat boolean. should matching stats be computed
#' @return A data.frame with matched pairs (row of reference positions in first column, and row of detected positions in second column) and corresponding 3D distances
...
histDetection <- function(lr, ld, matched, plot=TRUE)
{
  ...
  if (plot)
  {
    ...
    graphics::legend("topright",c("True positive","False negative","False positive"), fill=c("green","red","blue"))
  }
  #
...
#' @export
#'
heightRegression <-function(lr, ld, matched, plot=TRUE, species=NULL, ...)
{
